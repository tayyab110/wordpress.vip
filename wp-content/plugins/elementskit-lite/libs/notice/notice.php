...
if ( ! class_exists( '\Oxaim\Libs\Notice' ) ) :
	...
	class Notice {
		...
		public static function dismiss_ajax_call() {
			...
			// whitelisting notice id
			$whitelists = ['elementskit', 'wpmet', 'shopengine', 'metform', 'wp-fundraising-donation', 'wp-social', 'wp-ultimate-review'];
			$whitelist_pattern = '/' . implode('|', array_map('preg_quote', $whitelists)) . '/i';
			if(!preg_match($whitelist_pattern, $notice_id)) {
				$notice_id = '';
			}
